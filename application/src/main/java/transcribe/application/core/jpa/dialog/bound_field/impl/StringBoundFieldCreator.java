...
public class StringBoundFieldCreator implements BoundFieldCreator {
    ...
    public <T, V> AbstractField<?, ?> newBoundField(PropertyDefinition<T, V> property,
                                                    Binder<T> binder,
                                                    boolean required) {
        var field = BeanUtils.isAnnotationPresentNested(property.getPropertyHolderType(), property.getName(), BigText.class)
