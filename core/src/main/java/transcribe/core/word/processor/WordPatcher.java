...
import transcribe.core.word.common.BaseSpeakerSegmentInfo;
import transcribe.core.word.common.BaseWordInfo;
...
public final class WordPatcher {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            switch (row.tag()) {
                ...
                case INSERT -> {
                    ...
                    W leftAnchor = TsLists.getSafe(patchedWords, patchedWords.size() - 1);
                    W rightAnchor = TsLists.getSafe(original, originalIndex);
    ...
    private static <W extends WordInfo, C extends HasContent> List<DiffRow> generateDiffRows(List<W> original, List<C> revised) {
        var originalTokens = TsLists.collect(original, W::getContent);
        var revisedTokens = TsLists.collect(revised, C::getContent);
